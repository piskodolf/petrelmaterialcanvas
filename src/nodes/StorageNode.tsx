...
export const StorageNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  // Soft migration from old data
  let columns: MaterialColumn[] = Array.isArray(data.columns) ? data.columns : [];
  if (columns.length === 0 && Array.isArray(data.materials) && data.materials.length > 0) {
    columns = data.materials.map(mat => ({
      materialUrl: mat,
      capacity: (data.storageCount as number) || 1,
      items: Array((data.storageCount as number) || 1).fill(null)
    }));
  }
  ...
  const updateColumnCapacity = (colIndex: number, delta: number) => {
    ...
    setEdges((eds) => eds.filter(e => {
      ...
      if (!handleId || (!handleId.startsWith('input-col-') && !handleId.startsWith('output-col-'))) return true;
      ...
      if (newCapVal === 0) {
        ...
        const handleId = isTarget ? e.targetHandle : e.sourceHandle;
        if (handleId && (handleId.startsWith('input-col-') || handleId.startsWith('output-col-'))) {
          const parts = handleId.split('-');
          const handlePrefix = parts[0];
          const edgeColIndex = parseInt(parts[2], 10);
  ...
  const removeFromNode = () => {
    ...
    const { colIndex, slotIndex } = lightboxData;
    setNodes((nds) => nds.map((n) => {
      if (n.id === id) {
        const cols = Array.isArray(n.data.columns) ? [...n.data.columns] : columns;
        if (slotIndex === null) {
    ...
    return (
      <div className="material-picker nodrag nopan">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '12px' }}>
          <button className="picker-upload-btn" onClick={() => {
  ...
  const renderColumns = (cols: MaterialColumn[]) => {
    return (
      <div className="process-columns-container" style={{ flexWrap: 'wrap', justifyContent: 'center' }}>
          ...
          <div key={colIndex} className="process-column">
            ...
            <div 
              ...
              onClick={() => {
                ...
                else setLightboxData({ src: col.materialUrl, colIndex, slotIndex: null });
              }}
              title="Tip materiala"
            >
              {col.materialUrl ? (
  ...
  return (
    <>
      ...
      <div className="detail-row" style={{ flexDirection: 'column', alignItems: 'flex-start', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '8px', margin: '0 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '4px' }}>
          <FileText size={12} className="detail-icon" style={{ color: 'var(--text-muted)' }} />
          <span className="detail-label" style={{ fontSize: '10px' }}>Opis skladišča:</span>
        </div>
        <textarea 
      ...
      <div className="storage-node-body" style={{ padding: '16px' }}>
